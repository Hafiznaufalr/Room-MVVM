...
//    val pagedSubscribers = repository.pagedSubscribers
    ...
    private fun update(subscriber: Subscriber): Job = viewModelScope.launch {
        ...
        if (noOfRows > 0) {
            ...
            saveOrUpdateButtonText.value = "Save"
            clearAllOrDeleteButtonText.value = "Clear All"
            statusMessage.value = Event("$noOfRows Row Subscriber Updated Successfully")
        } else {
            statusMessage.value = Event("Error Occurred")
        }
    ...
    private fun delete(subscriber: Subscriber): Job = viewModelScope.launch {
        ...
        if (noOfRowsDeleted > 0) {
            ...
            saveOrUpdateButtonText.value = "Save"
            clearAllOrDeleteButtonText.value = "Clear All"
            statusMessage.value = Event("$noOfRowsDeleted Row Subscriber Deleted Successfully")
        } else {
            statusMessage.value = Event("Error Occurred")
        }
    ...
    private fun clearAll(): Job = viewModelScope.launch {
        ...
        if (noOfRowsDeleted > 0) {
            statusMessage.value = Event("$noOfRowsDeleted Row All Subscribers Deleted Successfully")
        } else {
            statusMessage.value = Event("Error Occurred")
        }
    }
